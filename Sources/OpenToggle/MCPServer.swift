...
enum MCPServer {
    ...
    private static let toolDefinitions: [[String: Any]] = [
        ...
        tool("opentoggle_set_switch",
             "Turn a switch on or off. Works on hidden switches too.",
        ...
        tool("opentoggle_set_enabled",
             "Show or hide a switch in the menu bar panel. Hiding is visibility-only: a RUNNING switch keeps running (and keeps restoring at launch) — call opentoggle_set_switch with on=false first if you intend to stop it.",
             properties: ["id": strProp("Switch id"),
                          "enabled": ["type": "boolean", "description": "true = show in menu bar, false = hide (does NOT stop the switch)"]],
             required: ["id", "enabled"]),
